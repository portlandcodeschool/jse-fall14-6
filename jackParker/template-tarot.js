...
var Card = (function(){
	...
	Card.isCard = function(card) {
		if((typeof card.id) !== 'number' || (card.id < 0) || (card.id > this.numCards() - 1)) return false;
	...
	var cardProto = Card.prototype;
	
	
	// ===== is valid =====
	cardProto.isValid = function() {
		if(((typeof this.id) !== 'number') || (this.id < 0) || (this.id > (this.constructor.numCards() - 1))) return null;
		return true;
	};
	
	
	// ==== rank =====
	cardProto.rank = function() {
	...
	cardProto.name = function() {
		...
		rankNames = this.constructor.rankNames();
		suitNames = this.constructor.suitNames();
